...
const MyCliquesPage: React.FC<MyCliquesPageProps> = ({}) => {
  const classes = useStyles();
  const [moduleId, setModuleId] = useState(0);

  const [{ data, fetching }] = useMyGroupsQuery({
    ...
    <Layout variant="regular">
      <Grid container spacing={2} justify="flex-start" direction="column">
        <Grid container item xs={12}>
          ...
          <NextLink href={`/class/cliques/new`}>
            <Button
              variant="contained"
              color="primary"
...
export default withUrqlClient(CreateUrqlClient, { ssr: false })(MyCliquesPage);
